...
// int can_split_helper(node *root,int size)
// {
	
// 	if (!root)
// 		return (0);
// 	if (count(root) == size - count(root))
// 		return(1);
// 	return (((can_split_helper(root->left,size)) || (can_split_helper(root->right,size))));

// }

// int can_split(node *root)
// {
// 	int size = count(root);
	
// 	return (can_split_helper(root, size));
// }
